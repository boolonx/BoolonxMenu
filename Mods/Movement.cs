...
namespace BoolonxMenu.Mods
{
    public class Movement
    {
        ...
        private static GameObject CreatePlatformOnHand(Transform handTransform)
        {
            ...
            GameObject plat = GameObject.Instantiate(Plugin.brinePlatforms);
            plat.transform.localScale = new Vector3(0.025f, 0.3f, 0.4f) * GorillaTagger.Instance.offlineVRRig.scaleFactor;
            plat.AddComponent<GorillaSurfaceOverride>().overrideIndex = 109;  
            plat.transform.position = handTransform.position;
            plat.transform.rotation = handTransform.rotation;
            plat.layer = Noclip.clipping ? LayerMask.NameToLayer("Water") : LayerMask.NameToLayer("Default");
